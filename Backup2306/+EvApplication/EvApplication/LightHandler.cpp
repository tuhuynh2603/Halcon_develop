...
//	CString strTempName[] = {"TOP camera", "THR camera"};
	...
	for (int nTrack = 0; nTrack < m_pApp->m_nTotalTracks; nTrack++) {
		...
		//		strTemp.Format("T%d", nDoc);
				...
				strTempName[nViews] = m_pApp->m_strTHRCameraName[nDoc];
			}
...
int CLightHandler::LoadLightSettings(int nDocId, int nImage, BOOL bLightDlgOpen)
{
	...
	if(nImage == -1) {
		for(nImage=0; nImage<m_nImageCount; nImage++) {
			for(int nController=0; nController<m_nControllerCount; nController++)
				m_pController[nController].LoadLightSetting(nController, m_pApp->m_pTracks[nImage].m_pDoc[nDocId]->m_strConfigDir, nTempDoc, bLightDlgOpen, 0);
		}
		nImage = -1;
	}
	else {
		for (int nController = 0; nController < m_nControllerCount; nController++) {
			...
			m_pController[nController].LoadLightSetting(nController, m_pApp->m_pTracks[nImage].m_pDoc[nDocId]->m_strConfigDir, nTempDoc, bLightDlgOpen, bTrackSegments);
		}
	}
...
int CLightHandler::LoadLightSettingsFOV(int nDocId, int nImage, BOOL bLightDlgOpen, int nFov)
{
	int nView = 0;
	for (int i = 0; i < nImage + 1; i++) {
		for (int nDoc = 0; nDoc < m_pApp->m_pTracks[i].m_nTotalDocs; nDoc++) {
	...
	for (int nController = 0; nController < m_nControllerCount; nController++) {
		...
		m_pController[nController].LoadLightSettingFOV(nController, m_pApp->m_pTracks[nImage].m_pDoc[nDocId]->m_strConfigDir, nView, bLightDlgOpen, bTrackSegments, nFov);
	}
...
int CLightHandler::SaveLightSettings(int nDocId, int nImage)
{
	...
	if(nImage == -1) {
		for(nImage=0; nImage<m_nImageCount; nImage++) {
			for(int nController=0; nController<m_nControllerCount; nController++)
				m_pController[nController].SaveLightSetting(nController, m_pApp->m_pTracks[nImage].m_pDoc[nDocId]->m_strConfigDir, nTempDoc);
		}
	}
	else {
		for(int nController=0; nController<m_nControllerCount; nController++)
...
int CLightHandler::SaveLightSettingsFOV(int nDocId, int nImage, int nFov)
{
	int nView = 0;
	for (int i = 0; i <= nImage; i++) {
		for (int nDoc = 0; nDoc < m_pApp->m_pTracks[i].m_nTotalDocs; nDoc++) {
	...
	for (int nController = 0; nController < m_nControllerCount; nController++){
		m_pController[nController].SaveLightSettingFOV(nController, m_pApp->m_pTracks[nImage].m_pDoc[nDocId]->m_strConfigDir, nView, nFov);
	}
