...
BOOL CTabCtrlSSL::CheckDialogTemplate (LPCTSTR pszTemplateName) {
	...
	DLGTEMPLATEEXX* pTemplate = (DLGTEMPLATEEXX*) LockResource (hTemplate);
...
//void CTabCtrlSSL::GetOnSelChanging (NMHDR* pNMHDR, LRESULT* pResult) {
...
void CTabCtrlSSL::GetOnSelChange (NMHDR* pNMHDR, LRESULT* pResult) {
	int nIndex = GetCurSel ();
	if (nIndex == -1)
		return;
