...
//				m_dlgPaneStats->m_wndStatsDlg.UpdateTotalLotInfo();
				pApp->AddLogEvent("Sherlock_Spyglass Vision Statistics Saved");
			}
			
		}

		if(AfxMessageBox("Confirm to close Sherlock_Spyglass Vision application", MB_ICONQUESTION | MB_OKCANCEL) == IDOK) {
			pApp->AddLogEvent("Sherlock_Spyglass Vision Application closed");
		}
		else
			return;
	}

	//Closing application Msg
	if(theApp.m_pMsgBox != NULL)
		theApp.m_pMsgBox->Msg("Closing ...");
...
void CMainFrame::ShowPreviousImages()
{
	...
	if(!m_dlgPane){
		...
		if (!m_dlgPane->Create("Die View", this, CRect(0,0,800,400), TRUE,
			IDD_IMAGESELECTION_DLG, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_BOTTOM | CBRS_FLOAT_MULTI))
		{
		...
		m_dlgPane->DockToWindow(m_dlgPaneStats,CBRS_RIGHT,CRect(0,0,cx,cy));
...
void CMainFrame::ShowStatsPane()
{
	...
	CRect rectClient;
	GetDockingManager()->GetClientAreaBounds(rectClient);
	int cx = (int)(rectClient.Width() / 3);
	int cy = (int)(rectClient.Height() / 3);
	cy = cy + 50;
	cx = cx - 30;
	if(!m_dlgPaneStats){
		m_dlgPaneStats = new CDialogPaneStatsview;
			UINT style = WS_CHILD | CBRS_RIGHT |CBRS_FLOAT_MULTI;
		if (!m_dlgPaneStats->Create("Statistics", this,CRect(0,0,200,400), TRUE,
		...
		DockPane((CBasePane*)m_dlgPaneStats,AFX_IDW_DOCKBAR_BOTTOM, CRect(0, 0, cx, cy));
	...
	if (m_dlgPaneStats->bRefreshPane) {
		...
		m_dlgPaneStats->m_wndStatsDlg.UpdateTotalContents();
...
void CMainFrame::DisplayOpticsPanel(BOOL bVisible, int nHeight)
{
	if (bVisible) {
		DockControlBar(&m_wndOpticsPanel);
		if (m_wndOpticsPanel.m_nCurrentTrack == STATION_TOP) {
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV1)->ShowWindow(TRUE);
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV2)->ShowWindow(TRUE);
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV3)->ShowWindow(TRUE);
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV4)->ShowWindow(TRUE);
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV5)->ShowWindow(TRUE);
			m_wndOpticsPanel.GetDlgItem(IDC_STATIC_FOV_MODE)->ShowWindow(TRUE);
		}
		else {
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV1)->ShowWindow(FALSE);
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV2)->ShowWindow(FALSE);
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV3)->ShowWindow(FALSE);
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV4)->ShowWindow(FALSE);
			m_wndOpticsPanel.GetDlgItem(IDC_RADIO_FOV5)->ShowWindow(FALSE);
			m_wndOpticsPanel.GetDlgItem(IDC_STATIC_FOV_MODE)->ShowWindow(FALSE);
		}
...
void CMainFrame::OnWindowPosChanging(WINDOWPOS* lpwndpos)
{
	...
	//else
	lpwndpos->flags = lpwndpos->flags | SWP_NOSIZE | SWP_NOMOVE;
