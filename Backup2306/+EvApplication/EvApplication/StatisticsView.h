...
protected:
	...
	void UpdateUPH(double dStripTime, double dNetStripTime, int nDieNo);
	...
	void UpdateStatistics(size_t  GoodDieCount, size_t  BdDieCount);
	void UpdateStatisticsOnReScan(size_t  GoodDieCount, size_t  BdDieCount);
	void UpdateStatisticsOnRepeatScan(size_t  GoodDieCount, size_t  BdDieCount);
	CRITICAL_SECTION m_CriticalSectionUpdate;
	int nPPMStripNo;
	int nPPMNetStripNo;
	double dAverageStripTimeMs;
	double dNetAverageStripTime;
	...
	// Text color for Defect List
	COLORREF m_colorInfo1[100][20];

	// Text color for summary List
	COLORREF m_colorInfo2[70][20];

	// Text color for Statistics Information
	COLORREF m_colorInfo3[70][20];

	// Text color for Total Information
	COLORREF m_colorInfo4[500][20];

	// Defect Code Color for Total Information Column 0
	COLORREF m_DefectCodeColor[500];
	// Position Started Row of PVI Defect Code
	int m_nStartedRowPVIDefectCode;
	// Id of PVI Defect Code
	int m_nPVIDefectCodeId[500];
