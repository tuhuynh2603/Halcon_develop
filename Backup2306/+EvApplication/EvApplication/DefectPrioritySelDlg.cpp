...
CDefectPrioritySelDlg::CDefectPrioritySelDlg(CWnd* pParent /*=NULL*/)
	...
	, m_bEnableInspectionPriority(FALSE)
{
...
void CDefectPrioritySelDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_ENABLE_INSP_PRIORITY, m_bEnableInspectionPriority);
}
...
void CDefectPrioritySelDlg::DefectPrioritySelectionFile(BOOL bSave)
{
	...
	if(!bSave && iniFile.ReadFile()) {
		...
		//Option to enable/disable the Inspection Priority
		strTemp.Format("Enable Inspection Priority");
		m_bEnableInspectionPriority = pApp->m_bEnableInspectionPriority = iniFile.GetBoolean(strKey, strTemp, FALSE);
	}
	else {
		strTemp.Format("Defect Count");
		...
		//Option to enable/disable the Inspection Priority
		pApp->m_bEnableInspectionPriority = m_bEnableInspectionPriority;
		strTemp.Format("Enable Inspection Priority");
		iniFile.SetBoolean(strKey, strTemp, pApp->m_bEnableInspectionPriority);

		iniFile.WriteFile();

		UpdatePriorityInspection();
	}
}
...
void CDefectPrioritySelDlg::UpdatePriorityInspection()
{
	
	m_mapInspPriority.clear();
	for (size_t i = 0; i < pApp->m_strDefectErrorCodesPriority.GetCount(); i++) {
		m_mapInspPriority.insert(std::pair<CString, int>(pApp->m_strDefectErrorCodesPriority.GetAt(i), i));
	}

	//Update Priority Inspection Parameter For All PVI Inspection

	CString strCam, strDefectDisplay;
	for (int nTrack = 0; nTrack < pApp->m_nTotalTracks; nTrack++) {
		strCam = !nTrack ? "Top" : "THR";
		for (int nDoc = 0; nDoc < pApp->m_nTotalViewDocs[nTrack]; nDoc++) {
			...
			for (int nFov = 0; nFov < nCurrentFOV; nFov++) {
				CInspectionHandler* pInspHandler = &pApp->m_pTracks[nTrack].m_pDoc[nDoc]->m_InspectionHandler[nFov];

				//// ********* Region Insp (PVI Inspection ) ********** ////
				pInspHandler->m_Inspection.m_bInspectionPriority = pApp->m_bEnableInspectionPriority;
				HTuple hPVIArea;
				if (pInspHandler->m_Inspection.m_TrainingData.hvec_TupleVectorPVI[0].GetSize() > 0)
					hPVIArea = pInspHandler->m_Inspection.m_TrainingData.hvec_TupleVectorPVI[0][0];
				if (hPVIArea != 0 && hPVIArea.Length() > 0) {
					for (int nArea = 0; nArea < hPVIArea; nArea++) {
						for (int nDefCount = 0; nDefCount < pInspHandler->m_Inspection.m_pPviInspectionSetup[nArea].nDefectCount; nDefCount++) {
							strDefectDisplay.Format("%s_%s", strCam, pInspHandler->m_Inspection.m_pPviInspectionSetup[nArea].m_pPviDefect[nDefCount].strDefectName);
							if (m_mapInspPriority.find(strDefectDisplay) != m_mapInspPriority.end()) {
								pInspHandler->m_Inspection.m_pPviInspectionSetup[nArea].m_pPviDefect[nDefCount].m_nInspPriority = m_mapInspPriority[strDefectDisplay];
							}
						}
					}
				}
			...
			CInspectionHandler* pInspHandler = &pApp->m_pTracks[nTrack].m_pDoc[nDoc]->m_InspectionHandler[0];

			strDefectDisplay = "Encap Glue";
			if (m_mapInspPriority.find(strDefectDisplay) != m_mapInspPriority.end()) {
				pInspHandler->m_Inspection.m_EncapParm.nGluePriority = m_mapInspPriority[strDefectDisplay];
			}

			strDefectDisplay = "Encap Contamination";
			if (m_mapInspPriority.find(strDefectDisplay) != m_mapInspPriority.end()) {
				pInspHandler->m_Inspection.m_EncapParm.nContaminationPriority = m_mapInspPriority[strDefectDisplay];
			}

			strDefectDisplay = "Encap Blowhole";
			if (m_mapInspPriority.find(strDefectDisplay) != m_mapInspPriority.end()) {
				pInspHandler->m_Inspection.m_EncapParm.nBlowHolePriority = m_mapInspPriority[strDefectDisplay];
			}

			strDefectDisplay = "Encap Void";
			if (m_mapInspPriority.find(strDefectDisplay) != m_mapInspPriority.end()) {
				pInspHandler->m_Inspection.m_EncapParm.nVoidPriority = m_mapInspPriority[strDefectDisplay];
			}

			strDefectDisplay = "Encap Wire And IC";
			if (m_mapInspPriority.find(strDefectDisplay) != m_mapInspPriority.end()) {
				pInspHandler->m_Inspection.m_EncapParm.nWireAndICPriority = m_mapInspPriority[strDefectDisplay];
			}

			strDefectDisplay = "Encap Bulge";
			if (m_mapInspPriority.find(strDefectDisplay) != m_mapInspPriority.end()) {
				pInspHandler->m_Inspection.m_EncapParm.nBulgePriority = m_mapInspPriority[strDefectDisplay];
			}
		}
	}
}