...
public:
	...
	void RefreshLightFOV(int nDoc, int nFov);
	int ResetLightSteps();
	long getDuplicateInspectionThreadId();
	long getInspectionThreadId();
	long getInspectionTOPThreadId();

	long getInspectionThreadCount();
	long getDuplicateInspectionThreadCount();

	CArray<int, int> m_nLightControllerId;
	//CArray<int, int> m_nLightStartSegmentNo;

	CUIntArray m_nSegmentsOfImage[MAX_TRACKS];
	CUIntArray m_nSelectedSegmentsOfImage[MAX_TRACKS];
	...
	int WaitForCompletion(CImageBuffer** pGrabbedImgBuf);
	int WaitForCompletionTHR(CImageBuffer** pGrabbedImgBuf);
	...
	//Inspection Thread Id For Global Level Inspection
	volatile unsigned long m_nInspectionThreadId;
	
	volatile unsigned long m_nDuplicateInspectionThreadId;


	CRITICAL_SECTION CriticalSectionScannedImage;

//	boost::array < HANDLE, 3> eventDoInspection;
//	boost::array < HANDLE, 3> eventInspectionDone;
	std::vector <HANDLE> eventDoInspection;
	std::vector <HANDLE> eventInspectionDone;

	std::vector <HANDLE> eventDuplicateInspection;
	std::vector <HANDLE> eventDuplicateInspectionDone;

	HANDLE eventHardwareGrabThreadKilled;
//	HANDLE m_eventScanning;
	HANDLE m_eventTrigCount;
	HANDLE m_eventGrabbing;
