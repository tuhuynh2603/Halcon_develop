...
void CAppTabView::SetTitle()
{
	...
	if(pTrack->m_nTrack== STATION_TOP){
		pApp->nTop = pApp->m_nTOP+1;
		strCam.Format("%s %s", pApp->m_bIsMasterApp ? "" : "Server", pApp->m_strTopCameraName.GetAt(nCamNo));
	}
	else if(pTrack->m_nTrack== STATION_THR){
		pApp->nTHR = pApp->m_nTHR+1;
	   strCam.Format("%s %s", pApp->m_bIsMasterApp ? "" : "Server",pApp->m_strTHRCameraName.GetAt(nCamNo));
	}
	/*else{
		pApp->nSide = pApp->nSide+1;
	   strCam.Format("%s %s", pApp->m_bIsMasterApp ? "" : "Server",pApp->m_strSideCameraName.GetAt(nCamNo));
	}*/

	strCam.Format("%s %s", pApp->m_bIsMasterApp ? "" : "Server", pTrack->m_nTrack == 0 ? pApp->m_strTopCameraName.GetAt(nCamNo) : pApp->m_strTHRCameraName.GetAt(nCamNo));
