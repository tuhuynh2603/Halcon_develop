...
BEGIN_MESSAGE_MAP(CMultipleRecipeDialog, CDialogEx)
	...
	ON_BN_CLICKED(IDC_MANAGE_PACKAGE_ID, &CMultipleRecipeDialog::OnBnClickedManagePackageId)
END_MESSAGE_MAP()
...
BOOL CMultipleRecipeDialog::OnInitDialog() 
{
	...
	if (theApp.m_bEnableManagePackageId)
		GetDlgItem(IDC_MANAGE_PACKAGE_ID)->EnableWindow(TRUE);
	else
		GetDlgItem(IDC_MANAGE_PACKAGE_ID)->EnableWindow(FALSE);
...
void CMultipleRecipeDialog::OnBnClickedManagePackageId()
{
	ManagePackageIdDialog managePackageIdDialog;

	managePackageIdDialog.DoModal();
}
