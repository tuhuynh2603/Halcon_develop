...
//		m_ctrlLinearEncoderPos.SetFont(&m_FontList);
		...
		m_ctrlMotorPos.InsertColumn(0, "Motor Name", LVCFMT_CENTER, 150, 0);
		...
		/*for(i = 0; i<nTotalScaleNo; i++) {
			lvitem.iItem = i + 1;
			m_ctrlLinearEncoderPos.InsertItem(&lvitem);
			pMotionParms->GetScaleName(i, strTemp);
			m_ctrlLinearEncoderPos.SetItemText(i, 0, strTemp);
			m_ctrlLinearEncoderPos.SetItemText(i, 1, "0");
			m_ctrlLinearEncoderPos.SetItemText(i, 2, "0");
		}*/
...
void CMotionControlDialog::OnCbnSelchangeMotorIdCombo()
{
	...
	switch(m_nMotorID) {
		case X_SERVO_MOTOR: //X-axis servo motor.
			m_PosMovement.LoadBitmaps(IDB_ARROWRIGHT, IDB_ARROWRIGHT_CLICKED);
			m_NegMovement.LoadBitmaps(IDB_ARROWLEFT, IDB_ARROWLEFT_CLICKED);
		...
		case WIDTH_ADJUSTMENT_STEPPER_MOTOR: //Width Adjustment stepper motor
			m_PosMovement.LoadBitmaps(IDB_ARROWOUTOF, IDB_ARROWOUTOF_CLICKED);
			m_NegMovement.LoadBitmaps(IDB_ARROWINTO, IDB_ARROWINTO_CLICKED);

			m_PosRelativeMovement.LoadBitmaps(IDB_ARROWOUTOF_SMALL, IDB_ARROWOUTOF_CLICKED_SMALL);
			m_NegRelativeMovement.LoadBitmaps(IDB_ARROWINTO_SMALL, IDB_ARROWINTO_CLICKED_SMALL);
			break;

		case Y_SERVO_MOTOR: //Y-axis servo motor
		...
		case PUSH_PULL_LINEAR_MOTOR: //Push pull linear Motor
			m_PosMovement.LoadBitmaps(IDB_ARROWRIGHT, IDB_ARROWRIGHT_CLICKED);
			m_NegMovement.LoadBitmaps(IDB_ARROWLEFT, IDB_ARROWLEFT_CLICKED);
		...
		case ST1_Z_SERVO_MOTOR: //Station 1 Z-axis servo motor
			m_PosMovement.LoadBitmaps(IDB_ARROWUP, IDB_ARROWUP_CLICKED);
			m_NegMovement.LoadBitmaps(IDB_ARROWDOWN, IDB_ARROWDOWN_CLICKED);
		...
		case ST2_Z_SERVO_MOTOR: //Station 2 Z-axis servo motor
			m_PosMovement.LoadBitmaps(IDB_ARROWUP, IDB_ARROWUP_CLICKED);
			m_NegMovement.LoadBitmaps(IDB_ARROWDOWN, IDB_ARROWDOWN_CLICKED);
	...
	if(m_nMotorIDPrev != m_nMotorID) {
		...
		pMotionParms->setScanningSpeedTHR(m_nMotorIDPrev, scanSpeedTHR);
...
void CMotionControlDialog::UpdateParameters()
{
	...
	if(scanSpeedTHR.startSpeed < 1)
		scanSpeedTHR.startSpeed = 1;
	if(scanSpeedTHR.maximumSpeed < 1)
		scanSpeedTHR.maximumSpeed = 1;
	...
	if(scanSpeedTHR.startSpeed > scanSpeedTHR.maximumSpeed)
		scanSpeedTHR.startSpeed = scanSpeedTHR.maximumSpeed;
	...
	pMotionParms->setScanningSpeedTHR(m_nMotorID, scanSpeedTHR);
