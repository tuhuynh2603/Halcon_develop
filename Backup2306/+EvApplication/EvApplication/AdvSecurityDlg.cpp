...
bool CAdvSecurityDlg::GetSecurityAccess()
{
	...
	m_nOlddwvalue[70] = m_data.m_nSaveOnlineJpegImages;
	...
	m_nOlddwvalue[71] = m_data.m_nSaveOnlineBMPImages;


	dwReturn = RegQueryValueEx(hKey, "rVerificationMode", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		// Default = 1(tech)
		dwValue = 1;
	}
	m_data.m_nVerificationMode = (int)dwValue;
	m_nOlddwvalue[72] = m_data.m_nVerificationMode;

	dwReturn = RegQueryValueEx(hKey, "rVerificationWindow", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		// Default = 1(tech)
		dwValue = 1;
	}
	m_data.m_nVerificationWindow = (int)dwValue;
	m_nOlddwvalue[73] = m_data.m_nVerificationWindow;
	...
	dwReturn = RegQueryValueEx(hKey, "rMotorTrigger", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		// Default = 2(Engineering)
		dwValue = 0;
	}
	m_data.m_nMotorTrigger = (int)dwValue;
	m_nOlddwvalue[52] = m_data.m_nMotorTrigger;


	dwReturn = RegQueryValueEx(hKey, "rTriangularSensor", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		// Default = 2(Engineering)
		dwValue = 0;
	}
	m_data.m_nTriangularSensor = (int)dwValue;
	m_nOlddwvalue[53] = m_data.m_nTriangularSensor;

	dwReturn = RegQueryValueEx(hKey, "rBarCodeReader", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		// Default = 2(Engineering)
		dwValue = 0;
	}
	m_data.m_nBarcodeReader = (int)dwValue;
	m_nOlddwvalue[54] = m_data.m_nBarcodeReader;
	...
	m_nOlddwvalue[55] = m_data.m_nZoomIn;
	...
	m_nOlddwvalue[56] = m_data.m_nZoomOut;
	...
	m_nOlddwvalue[57] = m_data.m_nZoomFit;
	...
	m_nOlddwvalue[58] = m_data.m_nZoom100;
	...
	m_nOlddwvalue[59] = m_data.m_nZoomAll;
	...
	m_nOlddwvalue[60] = m_data.m_nSetupGrid;
	...
	m_nOlddwvalue[61] = m_data.m_nShowOverLay;
	...
	m_nOlddwvalue[62] = m_data.m_nAuditTrail;
	...
	m_nOlddwvalue[63] = m_data.m_nShowMaginifier;
	...
	dwReturn = RegQueryValueEx(hKey, "rSequenceRun", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	
	if (dwReturn != ERROR_SUCCESS) {
		dwValue = 0;
	}
	m_data.m_nSequenceRun = (int)dwValue;
	m_nOlddwvalue[64] = m_data.m_nSequenceRun;


	dwReturn = RegQueryValueEx(hKey, "rNestReplacement", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		dwValue = 0;
	}
	m_data.m_nNestReplaceMent= (int)dwValue;
	m_nOlddwvalue[65] = m_data.m_nNestReplaceMent;

	

	dwReturn = RegQueryValueEx(hKey, "rBarCode", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		dwValue = 0;

	}
	m_data.m_nBarCode = (int)dwValue;
	m_nOlddwvalue[66] = m_data.m_nBarCode;


	dwReturn = RegQueryValueEx(hKey, "rClamp", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		dwValue = 0;

	}
	m_data.m_nClamp = (int)dwValue;
	m_nOlddwvalue[67] = m_data.m_nClamp;


	
	dwReturn = RegQueryValueEx(hKey, "rUnClamp", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		dwValue = 0;

	}
	m_data.m_nUnClamp = (int)dwValue;
	m_nOlddwvalue[68] = m_data.m_nUnClamp;

	
	dwReturn = RegQueryValueEx(hKey, "ScanningOption", NULL, &dwType, (unsigned char *)&dwValue, &dwLength);
	if (dwReturn != ERROR_SUCCESS) {
		dwValue = 0;

	}
	m_data.m_nScanningOptions = (int)dwValue;
	m_nOlddwvalue[69] = m_data.m_nScanningOptions;
...
bool CAdvSecurityDlg::SetSecurityAccess()
{
	...
	if (m_nOlddwvalue[70] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "SaveJpegImages", m_nOlddwvalue[70]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	if (m_nOlddwvalue[71] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "SaveBMPImages", m_nOlddwvalue[71]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	dwValue = (DWORD)m_data.m_nVerificationMode;
	dwReturn = RegSetValueEx(hKey, "rVerificationMode", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[72] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "VerificationMode", m_nOlddwvalue[72]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	dwValue = (DWORD)m_data.m_nVerificationWindow;
	dwReturn = RegSetValueEx(hKey, "rVerificationWindow", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[73] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "VerificationWindow", m_nOlddwvalue[73]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	dwValue = (DWORD)m_data.m_nMotorTrigger;
	dwReturn = RegSetValueEx(hKey, "rMotorTrigger", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[52] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "MotorTrigger", m_nOlddwvalue[51]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	dwValue = (DWORD)m_data.m_nTriangularSensor;
	dwReturn = RegSetValueEx(hKey, "rTriangularSensor", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[53] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "TriangularSensor", m_nOlddwvalue[51]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	dwValue = (DWORD)m_data.m_nBarcodeReader;
	dwReturn = RegSetValueEx(hKey, "rBarCodeReader", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[54] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "BarCodeReader", m_nOlddwvalue[51]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	dwValue = (DWORD)m_data.m_nSequenceRun;
	dwReturn = RegSetValueEx(hKey, "rSequenceRun", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[64] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "SequenceRun", m_nOlddwvalue[61]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	dwValue = (DWORD)m_data.m_nNestReplaceMent;
	dwReturn = RegSetValueEx(hKey, "rNestReplacement", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[65] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "NestReplacement", m_nOlddwvalue[62]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	dwValue = (DWORD)m_data.m_nBarCode;
	dwReturn = RegSetValueEx(hKey, "rBarCode", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[66] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "BarCode", m_nOlddwvalue[63]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	...
	dwValue = (DWORD)m_data.m_nClamp;
	dwReturn = RegSetValueEx(hKey, "rClamp", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[67] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "Clamp", m_nOlddwvalue[64]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	}
	dwValue = (DWORD)m_data.m_nUnClamp;
	dwReturn = RegSetValueEx(hKey, "rUnClamp", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[68] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "UnClamp", m_nOlddwvalue[65]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
	}
	dwValue = (DWORD)m_data.m_nScanningOptions;
	dwReturn = RegSetValueEx(hKey, "ScanningOption", NULL, REG_DWORD, (unsigned char *)&dwValue, 4);
	if (m_nOlddwvalue[69] != dwValue) {
		strMsg = GetAccsLevelStatus(dwValue, "ScanningOption", m_nOlddwvalue[66]);
		pApp->AddLogEvent(strMsg);
	}
	if (dwReturn != ERROR_SUCCESS) {
		::AfxMessageBox("Unable To Update Value");
