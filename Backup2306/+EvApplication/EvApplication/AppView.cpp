...
BOOL CAppView::OnMouseWheel(UINT nFlags, short zDelta, CPoint point)
{
	...
	if(pApp->m_bZoomAll) {
		...
		//	CAppDoc* pDoc = GetDocument();
		//	int nTrack = pDoc->m_pTrack->m_nTrack;
				pView = pApp->m_pTracks[j].m_pDoc[0]->m_ImageViewInfo.pImageView;
				if(pView != this)
...
void CAppView::OnDropFiles(HDROP hDropInfo)
{
	TCHAR* FileBuffer;
	UINT BufferSize;
	CString strFileName;
	...
	BufferSize = DragQueryFile(hDropInfo, 0, NULL, 0) + 1;
	FileBuffer = new TCHAR[BufferSize];

	// Get file name of dragged object
	DragQueryFile(hDropInfo, 0, FileBuffer, BufferSize);

	// Get file name and file extension
	strExtension = CString(PathFindExtension(FileBuffer) + 1).MakeLower();
	strFileName = CString(FileBuffer);
	delete FileBuffer;

	strMsg.Format(_T("Invalid File Type. Only .bmp is supported"));

	if (strExtension == "bmp") {
		CAppDoc* pDoc = GetDocument();
		pDoc->LoadStaticImage(strFileName);
	}
	else
		MessageBox(strMsg);

	CEvImageView::OnDropFiles(hDropInfo);
}