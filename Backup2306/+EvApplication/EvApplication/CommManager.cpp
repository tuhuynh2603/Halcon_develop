	...
	switch(nCommandTemp){
		case INFO:{
					 // if(!m_pApp->m_bOnline)
					  ...
					  if(m_pApp->m_bOnline && !m_pApp->m_bCurtainSensor && !m_pApp->m_bEmergencyStop){
		...
		case START:{ // strMessage =  ",'3456789',3456789','3456789',1,1,1,'345uknn','receipe'";
						...
						m_pApp->AddLogEvent(strCommEventMsg, COMM_LOG);
						i=1;
						for(int j=0;j<m_pApp->m_nNoOfDies;j++,i++)
						{
						...
						if(m_pApp->m_bCurtainSensor || m_pApp->m_bEmergencyStop || m_pApp->m_bReceivedStopCmd)
						{
							...
							for(int k=1;k<m_pApp->m_nNoOfDies;k++)
							{
		...
		case RESULT:{
						...
						strTemp3.Format("%d",m_pApp->m_nPartErrorCode[0]);
						for(int k=1;k<m_pApp->m_nNoOfDies;k++)
						{
							strTemp2.Format("%s,%d",strTemp3,m_pApp->m_nPartErrorCode[k]);
							strTemp3 = strTemp2;
						}
