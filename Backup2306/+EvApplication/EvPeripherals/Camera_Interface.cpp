...
int RegisterCameraCard(CCameraInterface *pFGM, CString strCameraCard)
{
	if(strCameraCard.CompareNoCase("none")!=0) {
		...
		OutputDebugLogTo(0, FALSE, "Registering Camera Card %s...", strCameraCard);
		...
		pFGM->lpEnableOutputLine = (ENABLE_OUTPUT_LINE) GetProcAddress(pFGM->hDLL, "EnableOutputLine");
		if(pFGM->lpEnableOutputLine == NULL)
			OutputDebugLogTo(0, FALSE, "Fail to register EnableOutputLine()");
...
int ReleaseCamera(CCameraInterface *pFGM)
{
	...
	if(pFGM->lpReleaseCamera) {
		if(pFGM->pCamModule) {
			...
			pFGM->pCamModule = NULL;
		}
		pFGM->lpReleaseCamera = NULL;
	}
	return 0;
}
...
int SetTrigMode(CCameraInterface *pFGM,int nMode, int m_nCameraTrigDelay)
{
	int nError = 0;

	if(pFGM->lpSetTrigMode)
		nError = pFGM->lpSetTrigMode(pFGM->pCamModule,nMode, m_nCameraTrigDelay);
		if(nError) {
...
{
	if(pFGM->lpEnableOutputLine)
		return pFGM->lpEnableOutputLine(pFGM->pCamModule, enable);
