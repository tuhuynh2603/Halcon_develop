...
void SwitchImg(IMAGEVIEW_INFO *pImageViewInfo, BOOL bBinaryMode, int nThreshold)
{
	(pImageViewInfo->pImageView)->SendMessage(WM_BINARY_MODE,(WPARAM)bBinaryMode,(LPARAM)nThreshold);
}

int GetImgThreshold(IMAGEVIEW_INFO *pImageViewInfo)
{
	CEvImageDoc *pDoc = pImageViewInfo->pImageView->GetDocument();
	return pDoc->m_nThreshold;
}
