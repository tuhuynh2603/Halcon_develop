...
int CEvTeachPropertiesWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// Create combo:
	const DWORD dwViewStyle = WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | CBS_AUTOHSCROLL|WS_BORDER | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_VSCROLL;
	...
	for(int i=0; i<m_nPos; i++) {
		...
		for(int j=0; j<m_nSets; j++) {
			...
			if(!i) {
				...
				else if(j>=m_nTopSets && j< (m_nTopSets+m_nTHRSets)){
					str.Format("%s", strTHRName.GetAt(nTHRNo));
					nTHRNo++;
				}
