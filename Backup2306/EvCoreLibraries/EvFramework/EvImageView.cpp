...
CEvImageView::CEvImageView()
{
	...
	// Polygon Dragging
	m_bEnableCreateDragPolygon = FALSE;
	m_bEnablePolygonDragging = FALSE;
	m_ptPreCenterDragPolygon = CPoint(0, 0);
	m_nShiftPolygonAlongX = 0;
	m_nShiftPolygonAlongY = 0;
}
...
void CEvImageView::OnDraw(CDC* pDC)
{
	...
	if (pDoc->m_bTrackRectangle) {
		...
		// Create polygon dragging only once time
		if (m_bEnableCreateDragPolygon) {
			CRect rect_DragPolygon;
			// Find smallest rectangle contain all polygon point
			findSmallestRectangle(&rect_DragPolygon);

			// Add rectangle fop Track arry
			TRACK_INFO DragTracker;
			DragTracker.rect = rect_DragPolygon;
			//Tracker Type: 0-Rectangle, 1-Polygon, 2-Eclipse, 3-Polyline, 4-PolygonDragging
			DragTracker.nTrackerType = 4;
			m_RectTrackAry.Add(DragTracker);

			m_ptPreCenterDragPolygon = rect_DragPolygon.CenterPoint();
		...
		// Check to drag Polygon or resize Polygon
		if (m_bEnablePolygonDragging == TRUE && pDoc->m_nTrackerIdx == m_RectTrackAry.GetSize() - 1 && polylineId.GetSize() > 0) {
			// Move all Point of Polygon in a smallest Rectangle
			int nRectTrackArySize = m_RectTrackAry.GetSize();
			CPoint ptCurCenterDragPolygon = m_RectTrackAry.ElementAt(nRectTrackArySize - 1).rect.CenterPoint();
			m_nShiftPolygonAlongX = ptCurCenterDragPolygon.x - m_ptPreCenterDragPolygon.x;
			m_nShiftPolygonAlongY = ptCurCenterDragPolygon.y - m_ptPreCenterDragPolygon.y;

			for (int k = 0; k < m_RectTrackAry.GetSize() - 1; k++) {
				CRect tmpTrackRect = m_RectTrackAry.ElementAt(k).rect;
				tmpTrackRect.top += m_nShiftPolygonAlongY;
				tmpTrackRect.bottom += m_nShiftPolygonAlongY;
				tmpTrackRect.left += m_nShiftPolygonAlongX;
				tmpTrackRect.right += m_nShiftPolygonAlongX;
				m_RectTrackAry.ElementAt(k).rect = tmpTrackRect;
			}

			m_ptPreCenterDragPolygon = ptCurCenterDragPolygon;
		}
		else if (m_bEnablePolygonDragging == TRUE && polylineId.GetSize() > 0) {
			// Resize smallest rectangle which contains all point of Polygon
			CRect rect_DragPolygon;
			findSmallestRectangle(&rect_DragPolygon);
			int nRectTrackArySize = m_RectTrackAry.GetSize();
			m_RectTrackAry.ElementAt(nRectTrackArySize - 1).rect = rect_DragPolygon;
			m_ptPreCenterDragPolygon = m_RectTrackAry.ElementAt(nRectTrackArySize - 1).rect.CenterPoint();
		}
...
LRESULT CEvImageView::BinaryMode(WPARAM wParam, LPARAM lParam)
{
	...
	if (pDoc->m_bBinaryMode) {
		if (!m_pdlgThres) {
			...
			m_pdlgThres->m_nThresholdValue = pDoc->m_nThreshold;
			m_pdlgThres->CreateNew(this);
			m_pdlgThres->ShowWindow(SW_SHOW);
		}
		pDoc->m_pbufImage->GetAdapter()->ChangeToBinaryPalette(pDoc->m_nThreshold);
	}
	else {
		if(m_pdlgThres) {
...
LRESULT CEvImageView::ViewDrwTrackRect(WPARAM wParam, LPARAM lParam)
{
	...
	// Check after finish track polygon and delete rectangle polygon dragging
	int nSize = m_RectTrackAry.GetSize();
	if (nSize > 0 && m_RectTrackAry.GetAt(nSize - 1).nTrackerType == 4) {
		m_RectTrackAry.RemoveAt(nSize - 1);
	}
	...
	// Enable to create Rectangle for Polygon dragging. Create only once time
	if (Tracker.nTrackerType == 1) {
		m_bEnableCreateDragPolygon = TRUE;
		m_bEnablePolygonDragging = TRUE;
	}
	else {
		m_bEnableCreateDragPolygon = FALSE;
		m_bEnablePolygonDragging = FALSE;
...
// Function find smallest rectangle for polygon dragging
void CEvImageView::findSmallestRectangle(CRect *rect_DragPolygon) {
	
	int nRectTrackArySize = m_RectTrackAry.GetSize();
	CPoint tmpPoint = m_RectTrackAry.ElementAt(0).rect.CenterPoint();
	int nMinX, nMaxX, nMinY, nMaxY;
	nMinX = tmpPoint.x;
	nMaxX = tmpPoint.x;
	nMinY = tmpPoint.y;
	nMaxY = tmpPoint.y;
	for (int k = 1; k < m_RectTrackAry.GetSize(); k++) {
		tmpPoint = m_RectTrackAry.ElementAt(k).rect.CenterPoint();
		if (nMinX > tmpPoint.x)
			nMinX = tmpPoint.x;
		if (nMaxX < tmpPoint.x)
			nMaxX = tmpPoint.x;
	...
	if ((nMaxX - nMinX) < 20 || (nMaxY - nMinY) < 20) {
		nMinX -= 10;
		nMaxX += 10;
		nMinY -= 10;
		nMaxY += 10;
	...
	*rect_DragPolygon = CRect(nMinX, nMinY, nMaxX, nMaxY);
}