...
//	delete m_pwndDefectProperties;
	if (m_pwndTeachProperties != NULL) {
		delete m_pwndTeachProperties;
		m_pwndTeachProperties = NULL;
	}
...
///	delete m_pDebugViewPane;
	...
	if (m_wndOutputPane != NULL) {
		delete m_wndOutputPane;
		m_wndOutputPane = NULL;
	}
}
...
{
	if(m_wndOutputPane != NULL && m_wndOutputPane->m_hWnd)
...
void CEvMainFrame::OnShowOutputLog()
{
	...
	//RecalcLayout();
		...
		if(!m_wndOutputPane){
			...
			if (!m_wndOutputPane->Create("Log Information", this, CRect(0, 0, 200, 500), TRUE, ID_VIEW_OUTPUTWND, WS_CHILD | CBRS_RIGHT | CBRS_FLOAT_MULTI,  /*AFX_CBRS_OUTLOOK_TABS*/AFX_CBRS_REGULAR_TABS, dwNoCloseBarStyle))
			{
			...
			DockPane((CBasePane*)m_wndOutputPane, AFX_IDW_DOCKBAR_RIGHT, CRect(0, 0, cx, 2*cy));
		...
		if(m_wndOutputPane->m_hWnd) {
			...
			if(m_wndOutputPane->IsFloating()){
				...
				//m_wndOutputPane->DockToWindow(GetDebugPane(), AFX_IDW_DOCKBAR_RIGHT,CRect(cx/2,cy/2,cx,cy));
			}
			m_wndOutputPane->ShowPane(TRUE, FALSE, TRUE);
