...
int CImageBuffer::Allocate(int width, int height, bool bEnableDisplay, bool bColor)
{
	if(width < 1 || height < 1) {
		if(m_bAllocated)
			Free();
	...
	if(m_bAllocated) {
		if((width == m_ImgSize.cx) &&
			(height == m_ImgSize.cy) &&
			(bEnableDisplay == m_bDisplayEnabled) &&
			(bColor == (m_nBitCount == 24)))
			return 0;
		else
			Free();
	}
	...
	int	nBytes = bColor ? m_ImgSize.cx * 3 : m_ImgSize.cx;

	nBytes = nBytes % 4;
	if(nBytes > 0)
		nBytes = 4 - nBytes;

	m_nPitch = (bColor ? m_ImgSize.cx * 3 : m_ImgSize.cx) + nBytes;

	m_nBitCount = bColor ? 24 : 8;


	if(static_cast<__int64>(m_nPitch) * (m_ImgSize.cy) * (bColor ? 3 : 1) > static_cast<__int64>(1 << 15) * (1 << 16)) {
		bEnableDisplay = false;
		bAllowDraw = false;
	}
	else {
		bAllowDraw = true;
	...
	m_bDisplayEnabled = bEnableDisplay;
	if(m_bDisplayEnabled) {
		...
		m_pImg = (BYTE*) malloc(static_cast<__int64>(m_nPitch)*m_ImgSize.cy);
		m_pAdapter = NULL;
		m_bIndirect = false;
	}
...
{
	return Allocate(ImgSize.cx, ImgSize.cy, bDisplayEnabled, bColor);
...
int CImageBuffer::GetHeight() const
{
	return m_ImgSize.cy;
}

int CImageBuffer::GetWidth() const
{
	return m_ImgSize.cx;
}

bool CImageBuffer::IsReadyForDisplay() const
{
	return (m_pAdapter != NULL && m_bDisplayEnabled);
}
