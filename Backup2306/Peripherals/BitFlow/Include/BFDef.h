...
{
	BFOptNone = 0,
	BFOptPhysicalMemory = 1		// These must be individual bits so they can be combined
...
// Timing info
typedef struct _BFSignalTimeInfoRec
{
	BFU32	CXPXOffset;		// only used by CXP board in continous image header latch mode
	BFU32	CXPYOffset;		// only used by CXP board in continous image header latch mode
	BFU64	TimeStamp;
	BFBOOL	OverwriteError;
}BFSignalTimeInfoRec, *BFSignalTimeInfoPtr;
...
typedef enum _BOD_OPTIONS
{
	...
	BOD_SECOND_BRD			= 0x00000010,				// Tell the dialog it being used to open the second board in 2x DMA mode
} BOD_OPTIONS;
...
typedef	struct _VQTabRec
{
	...
	DMAChannel	DMAChannel;							// DMA channel.
...
{
    BFGPOut0	= 0x001,							// GPOut 0.
    BFGPOut1	= 0x002,							// GPOut 1.
    BFGPOut2	= 0x004,							// GPOut 2. 
    BFGPOut3	= 0x008,							// GPOut 3.
    BFGPOut4	= 0x010,							// GPOut 4.
    BFGPOut5	= 0x020,							// GPOut 5. 
    BFGPOut6	= 0x040,							// GPOut 6. 
	BFGPOut7	= 0x080,							// GPOut 7.
	BFGPOut8	= 0x100,							// GPOut 8.
    BFGPOut9	= 0x200,							// GPOut 9. 
    BFGPOut10	= 0x400,							// GPOut 10. 
	BFGPOut11	= 0x800,							// GPOut 11.
...
#define BFDISP_FORMAT_NORMAL		0x0000
#define BFDISP_FORMAT_RGB_SWAP		0x0002
#define BFDISP_FORMAT_PACKED		0x0004
#define BFDISP_FORMAT_PACKED_NOSWAP	0x0008
...
#define BFGN2HCTABSIZE						(32 * 1024)			// Number of horizontal CTab entries in CTab table on R64 family.
#define BFGN2VCTABSIZE						(128 * 1024)		// Number of vertical CTab entries in CTab table on R64 family.
...
typedef void (*BFCallBackFuncPtr) (Bd Board, BFU32 Num, PBFVOID pUserData);

typedef struct _CallBackThreadRec
{
	Bd			Board;		// handle to board
	BFU32		SignalType;	// Signal type that this is a call back four
	PBFVOID		pUserData;	// Pointer to user define structure
...
typedef enum _BFFormats
{
	BFFormatStandard = 1,				// Standard format video output
	BFFormatInterlaced = 2,				// Standard format video output
	BFFormat1X_1Y,						// One tap, same as Standard
	BFFormat1X2_1Y,						// Two taps, odd/even pixel
	BFFormat2X_1Y,						// Two taps, two segments
	BFFormat2XE_1Y,						// Two taps, two segments, right tap mirrored
	BFFormat2XM_1Y,						// Two taps, two segments, left tap mirrored
	BFFormat1X_2Y,						// Two taps, top/bottom
	BFFormat1X_2YE,						// Two taps, top/bottom, bottom inverted
	BFFormat1X3_1Y,						// Three taps, three segments
	BFFormat1X4_1Y,						// Four taps, four way interleaved
	BFFormat4X_1Y,						// Four taps, four segments
	BFFormat2X2_1Y,						// Four taps, two segments, odd/even pixel
	BFFormat2X2E_1Y,					// Four taps, two segments, odd/even pixel, right two mirrored
	BFFormat2X2M_1Y,					// Four taps, two segments, odd/even pixel, left two mirrored
	BFFormat1X2_2YE,					// Four taps, top/bottom, bottom inverted, odd/even pixel
	BFFormat2X_2YE,						// Four taps, top/bottom, bottom inverted, two segments
	BFFormat2XE_2YE,					// Four taps, top/bottom, bottom inverted, two segments, right mirrored (reads out from corners)
	BFFormat2XM_2YE,					// Four taps, top/bottom, bottom inverted, two segments, left mirrored (reads out from corners)
	BFFormat1X8_1Y,						// Eight taps, eight way interleaved
	BFFormat8X_1Y,						// Eight taps, eight segments
	BFFormat4X2_1Y,						// Eight taps, four segments, odd/even pixels
	BFFormat5X2_1Y,						// Eight taps, five segments, odd/even pixels
	BFFormat2X2E_2YE,					// Eight taps, top/bottom, bottom inverted, two segments, odd/even pixels, right two mirrored
	BFFormat1X10_1Y,					// Ten taps, ten way interleaved
	BFFormat10X_1Y,						// Ten taps, ten segments

	BFNumFormats						// Must be last
} BFFormats, *BFFormatsPtr;

typedef enum _BFScanTypes
{
	BFScanTypeArea,				// Area Scan
	BFScanTypeLine,				// Line Scan
	BFNumScanTypes				// Must be last
} BFScanTypes, *BFScanTypesPtr;

typedef enum _BFSyncModes
{
	BFSyncModeDefault,			// Normal sync mode
	BFSyncModeLinkResetWO,		// Special sync mode where Link Reset is WO
	BFNumSyncModes				// Must be last
} BFSyncModes, *BFSyncModesPtr;


// maximum size of the Timging Sequencer on Gen 2 boards
#define MAX_TS_INDEX	255


// Link list record used to send sequences to the Timing Sequencer
typedef struct _TSListkRec
{
    struct _TSListkRec      *Next;				// Link to next TS record or BFNULL.
	BFDOUBLE				Period;				// Time (in milliseconds) for this segment
	BFU32					CT0State;			// State of CT0 (0 or 1) for this segment
	BFU32					CT1State;			// State of CT1 (0 or 1) for this segment
	BFU32					CT2State;			// State of CT2 (0 or 1) for this segment
	BFU32					CT3State;			// State of CT3 (0 or 1) for this segment
	BFU32					WaitForTrigger;		// Trigger options to execute period
}TSListRec, *TSListPtr;
